...
def draw3DBox(surface, x, y, w, h, colorset):
  ...
  pygame.draw.line(surface, colorset.SubShadow, (x+w-1,y+1), (x+w-1,y+h-1))
  pygame.draw.line(surface, colorset.SubShadow, (x+1,y+h-1), (x+w-1,y+h-1))
...
class tButtonBase:
  ...
  def mouseMove(self, position):
    (x, y) = self.Position
    (mx, my) = position
    self.Pushed = self.Activated and x <= mx <= x + self.Width and y <= my <= y + self.Height
...
class tFloatUpAndFadeLabel:
  ...
  def render(self, surface):
    ...
    text_surf = SYSTEM_FONT_SMALL.get().render(self.Text, 1, (scaled_opacity,scaled_opacity,scaled_opacity,0 ))
    surface.blit(text_surf, self.Position)
