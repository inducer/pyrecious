...
def loadStones():
    for color in COLORS:
        STONE_IMAGES[color] = loadPictureList(os.path.join("gems", color))
        LIT_STONE_IMAGES[color] = loadPictureList(os.path.join("gems", color + "-lit"))
...
def loadFonts():
    SYSTEM_FONT_SMALL.set(pygame.font.Font(os.path.join("fonts", "verdanab.ttf"), 15))
    SYSTEM_FONT_BIG.set(pygame.font.Font(os.path.join("fonts", "verdanab.ttf"), 30))
